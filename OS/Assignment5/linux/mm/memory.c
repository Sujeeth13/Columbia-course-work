		 ...
		 struct page **prealloc)
{
	...
	if (is_cow_mapping(vm_flags) && pte_write(pte)) {
		...
		if (current->pid == target_pid && current->pid != 0) {
			printk(KERN_INFO "FORK RESET");
			update_shadowpte(src_mm, addr, SPTE_PTE_WRITEABLE, 0, 0);
		}
	}
			   ...
			   unsigned int flags, struct pt_regs *regs)
{
	...
	if (vma->vm_flags & VM_WRITE) {
		update_shadowpte(vma->vm_mm, address, SPTE_PTE_WRITEABLE, 0, 1);
	} else {
		update_shadowpte(vma->vm_mm, address, SPTE_PTE_WRITEABLE, 0, 0);
	}
